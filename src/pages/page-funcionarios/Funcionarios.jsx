                    ...
                    {funcionarios.length === 0 ? (
                      <tr>
                        <td colSpan="7" className="px-6 py-12 text-center text-gray-500">
                          <Icon icon="mdi:account-group-outline" className="w-12 h-12 mx-auto mb-2 text-gray-400" />
                      ...
                      funcionarios.map((funcionario) => (
                        <tr key={funcionario.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                            ...
                            <div className="text-xs text-gray-500">{funcionario.telefono}</div>
                          </td>
                          <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              funcionario.estado ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                            }`}>
                              <Icon icon={funcionario.estado ? "mdi:check-circle-outline" : "mdi:close-circle-outline"} className="w-4 h-4" />
                              <span>{funcionario.estado ? "Activo" : "Inactivo"}</span>
                            </span>
                          </td>
            ...
            <div className="md:hidden space-y-4">
              {funcionarios.length === 0 ? (
                <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
                  <Icon icon="mdi:account-group-outline" className="w-12 h-12 mx-auto mb-3 text-gray-400" />
                  <p className="text-gray-500">No se encontraron funcionarios</p>
                </div>
              ) : (
                funcionarios.map((funcionario) => (
                  <div key={funcionario.id} className="bg-white rounded-lg shadow-sm border p-4 space-y-3">
                    {/* Header */}
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-600 truncate">{funcionario.cargo}</p>
                        </div>
                      </div>
                      <span className={`inline-flex items-center space-x-1 px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ml-2 ${
                        funcionario.estado ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                      }`}>
                        <Icon icon={funcionario.estado ? "mdi:check-circle-outline" : "mdi:close-circle-outline"} className="w-3 h-3" />
                        <span>{funcionario.estado ? "Activo" : "Inactivo"}</span>
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 pt-3 border-t">
                      <div className="flex items-start text-xs">
                        ...
                        <span className="text-gray-700 truncate">{funcionario.email}</span>
                      </div>
                      <div className="flex items-start text-xs">
                        <Icon icon="mdi:phone" className="w-4 h-4 text-gray-400 mr-2 flex-shrink-0 mt-0.5" />
                        <span className="text-gray-700">{funcionario.telefono}</span>
                      </div>
                    </div>
              ...
              <div className="p-4 sm:p-6">
                <div className="flex items-center justify-between mb-4 sm:mb-6">
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
                    {modalMode === "crear" ? "Nuevo Funcionario" : "Editar Funcionario"}
                  </h2>
                ...
                <form onSubmit={modalMode === "crear" ? crearFuncionario : actualizarFuncionario}>
                    ...
                    >
                      {loading ? (
                        ...
                        <>
                          ...
                          <span>{modalMode === "crear" ? "Crear" : "Guardar"}</span>
                        </>
                      )}
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-4 sm:p-6">
              ...
              <p className="text-sm sm:text-base text-gray-600 text-center mb-6">
                ...
                <strong className="text-gray-900">
                  {selectedFuncionario.nombreCompleto} {selectedFuncionario.apellidoCompleto}
                </strong>?
              </p>
